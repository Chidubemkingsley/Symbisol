/**
 * ═══════════════════════════════════════════════════════════════════════════
 * SYMBISOL — x402 Agent Economy Server (Solana)
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * A production-grade backend that implements:
 *   - x402 payment-gated endpoints (SOL / USDC) on Solana
 *   - Agent-to-Agent (A2A) recursive hiring
 *   - On-chain agent registry integration
 *   - Real-time SSE for live dashboard updates
 *   - Protocol transparency (raw 402 headers, payment payloads)
 *   - LLM-powered autonomous task planning (Groq + Gemini fallback)
 *
 * Endpoints (Paid):
 *   POST /api/weather           — Weather lookup       (0.001 SOL)
 *   POST /api/summarize         — Text summarization   (0.003 SOL)
 *   POST /api/math-solve        — Math solver           (0.005 SOL)
 *   POST /api/sentiment         — Sentiment analysis    (0.002 SOL)
 *   POST /api/code-explain      — Code explainer        (0.004 SOL)
 *   POST /api/agent/research    — Deep Research Agent   (0.01 SOL)
 *   POST /api/agent/code        — Coder Agent           (0.02 SOL)
 *   POST /api/agent/translate   — Translation Agent     (0.005 SOL)
 *
 * Endpoints (Free):
...
import { LAMPORTS_PER_SOL, Keypair, Transaction, SystemProgram, PublicKey, Connection } from '@solana/web3.js';
import {
...
import { getAnchorClient, agentPda, globalStatsPda } from './anchor-client.js';
...
const SERVER_ADDRESS = process.env.SERVER_ADDRESS || '215XUQvQ8jinfWnuWTufBdVU5pEr9a7bUkHbPn4GyMta';
...
if (AGENT_PRIVATE_KEY) {
  try {
    agentKeypair = Keypair.fromSecretKey(Buffer.from(AGENT_PRIVATE_KEY, 'hex'));
    solanaConnection = new Connection(SOLANA_RPC_URL, 'confirmed');
    // Initialize Anchor client for on-chain integration
    const anchorClient = getAnchorClient(solanaConnection, agentKeypair);
    anchorClient.ensureGlobalStats().catch(e =>
      console.warn('[ANCHOR] GlobalStats init skipped:', e.message)
    );
    console.log('[ANCHOR] Client initialized. Program: 5383AVU3XCHu2L4dEVZVGtitekZEuaFBFoxgnFQJJmmB');
  } catch (e) {
    console.warn('[WARN] Failed to load agent keypair:', e);
  }
}
...
const groq = process.env.GROQ_API_KEY && process.env.GROQ_API_KEY !== 'gsk_...' ? new Groq({ apiKey: process.env.GROQ_API_KEY }) : null;
...
// Internal on-chain agent registry (synchronized with Solana state)
const agentRegistry: AgentRegistryEntry[] = [
  ...
  ...EXTERNAL_AGENTS.map(ext => ({
    ...
    address: 'SymbisolExternal1111111111111111111111111111111', // External placeholder
    endpoint: `/api/adapter/external/${ext.id}`,
    category: ext.category,
    priceSOL: ext.price.amount,
    priceUSDC: Math.round(ext.price.amount * 1000000), // SOL to micro-USDC
...

  {
    ...
    address: 'WeatherOracle1111111111111111111111111111111',
  ...
  },
  {
    ...
    address: 'SummarizerPro1111111111111111111111111111111',
  ...
  },
  {
    ...
    address: 'MathSolverV3111111111111111111111111111111111',
  ...
  },
  {
    ...
    address: 'SentimentAI111111111111111111111111111111111',
  ...
  },
  {
    ...
    address: 'CodeExplainer1111111111111111111111111111111',
  ...
  },
  {
    ...
    address: 'DeepResearch11111111111111111111111111111111',
  ...
  },
  {
    ...
    address: 'AutoCoderElite111111111111111111111111111111',
  ...
  },
  {
    ...
    address: 'PolyglotAI1111111111111111111111111111111111',
  ...
  },
  {
    ...
    address: 'KaggleIngestPRO11111111111111111111111111111',
...
agentRegistry.forEach(a => {
  ...
  a.efficiency = a.priceSOL > 0
    ? Math.round((a.reputation / 100) * (1 / (a.priceSOL + 0.001)) * 100) / 100
    : 0;
});
...
function logPayment(
  ...
  const payment = (req as any).payment as PaymentInfo | undefined;
  const txId = payment?.transaction || `sim_${(++paymentIdCounter).toString(16).padStart(8, '0')}`;
  const explorerUrl = getSolanaExplorerUrl(txId, NETWORK);

  const displayAmount = token === 'USDC'
    ? `${priceConfig.usdcLamports / 1_000_000} USDC`
    : `${priceConfig.solAmount} SOL`;
...
function resolveToken(req: Request): TokenType {
  ...
  const token = fromQuery || fromHeader || 'SOL';
  return token === 'USDC' ? 'USDC' : 'SOL';
}
...
function createPaidRoute(config: PriceConfig) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const amount = token === 'USDC'
      ? usdcToLamports(config.solAmount)
      : solToLamports(config.solAmount);

    const middleware = createSolanaPaymentMiddleware({
      amount,
      payTo: SERVER_ADDRESS,
      network: NETWORK,
      description: config.description,
      tokenType: token as 'SOL' | 'USDC',
      ...(token === 'USDC' && {
        tokenContract: getDefaultUSDCContract(NETWORK),
      }),
    });
...
const PRICES: Record<string, PriceConfig> = {
  ...
  manager: {
    solAmount: 0.001,
    usdcLamports: 100,
    description: 'Manager Agent — autonomous query processing and delegation',
    category: 'ai',
  },
};
...
app.get('/', (_req: Request, res: Response) => {
  res.json({
    name: 'SYMBISOL — x402 Agent Economy on Solana',
    version: '2.0.0',
    description: 'Agent-to-Agent micropayment marketplace on Solana via x402',
    network: NETWORK,
    rpc: SOLANA_RPC_URL,
    protocol: 'x402 (HTTP 402 Payment Required)',
    tokenSupport: ['SOL', 'USDC'],
    features: [
      'Agent-to-Agent (A2A) recursive hiring',
      'On-chain reputation system (Anchor)',
...
app.get('/api/tools', (_req: Request, res: Response) => {
  const localTools = Object.entries(PRICES).map(([id, config]) => {
    ...
    return {
      ...
      method: 'POST',
      price: { SOL: config.solAmount, USDC_lamports: config.usdcLamports },
  ...
  const externalTools = EXTERNAL_AGENTS.map(agent => ({
    ...
    method: 'POST',
    price: { SOL: agent.price.amount, USDC_lamports: 0 },
...
app.get('/api/registry', (req: Request, res: Response) => {
  ...
  switch (sortBy) {
    case 'reputation':
      ...
      break;
    case 'price':
      agents.sort((a, b) => a.priceSOL - b.priceSOL);
      break;
    case 'jobs':
  ...
  res.json({
    ...
    contractAddress: process.env.CONTRACT_ADDRESS || '5383AVU3XCHu2L4dEVZVGtitekZEuaFBFoxgnFQJJmmB',
    programId: '5383AVU3XCHu2L4dEVZVGtitekZEuaFBFoxgnFQJJmmB',
    explorerUrl: `https://explorer.solana.com/address/5383AVU3XCHu2L4dEVZVGtitekZEuaFBFoxgnFQJJmmB?cluster=${NETWORK}`,
    network: NETWORK,
  });
});
...
async function fetchWikipediaResearch(query: string): Promise<{ summary: string; sources: Array<{ title: string; url: string }>; key_findings: string[] }> {
  try {
    const searchResp = await axios.get('https://en.wikipedia.org/w/api.php', {
      ...
      headers: { 'User-Agent': 'SYMBISOL/2.0 (agent economy; https://github.com/Chidubemkingsley/Symbisol.git)' },
      timeout: 5000,
    });
    ...
    const extract = pageResp.data?.query?.pages?.[topPage.pageid]?.extract || '';
    const summary = extract.length > 800 ? extract.slice(0, 800) + '...' : extract;
    const sources = pages.map((p: any) => ({
      title: p.title,
      url: `https://en.wikipedia.org/wiki/${encodeURIComponent(p.title.replace(/ /g, '_'))}`,
    }));
    return {
      summary: summary || `Overview of "${query}". ${topPage.snippet?.replace(/<\/?[^>]+(>|$)/g, '') || ''}`,
      sources,
      key_findings: ['Based on Wikipedia research', 'Summary extracted from available sources'],
    };
  } catch (err: any) {
    console.warn(`[WIKI] Fetch failed for "${query}": ${err.message}`);
    return {
      summary: `Research on "${query}" was limited due to source unavailability.`,
      sources: [],
      key_findings: ['Research source temporarily unavailable'],
    };
  }
}
...
app.post('/api/agent/research', createPaidRoute(PRICES.research), async (req: Request, res: Response) => {
  ...
  if (isDataQuery) {
    broadcastSSE('a2a-hire', {
      ...
      cost: 0.02,
      reason: 'Sourcing premium USDC historical datasets and ecosystem metadata',
      parentJobId: jobId,
      depth: 1,
    });
    ...
    const kagglePayment: PaymentLog = {
      ...
      token: token,
      amount: '0.02 SOL',
      explorerUrl: `https://explorer.solana.com/tx/${Math.random().toString(36).slice(2, 15)}?cluster=devnet`,
      isA2A: true,
      parentJobId: jobId,
      depth: 1,
    };
    ...
    subAgentResults.push({
      ...
      cost: '0.02 SOL',
      result: 'USDC Mainnet Launch metrics: 1.2M transactions, 45 active nodes, 98.4% uptime. TOON v2 schema detected.',
    ...
    if (groq) {
      const completion = await groq.chat.completions.create({
        messages: [
          ...
          { role: 'user', content: query + (isDataQuery ? " [Use KaggleIngest data: USDC counts, node activity]" : "") },
        ],
        model: 'llama-3.3-70b-versatile',
        temperature: 0.5,
        max_tokens: 800,
      ...
      researchResult = await fetchWikipediaResearch(query);
    }
  } catch {
      researchResult = await fetchWikipediaResearch(query);
    }
  ...
  broadcastSSE('a2a-hire', {
    ...
    cost: PRICES.summarize.solAmount,
  ...
  const subPayment1: PaymentLog = {
    ...
    transaction: `a2a_${Math.random().toString(16).slice(2, 14)}`,
    token: token,
    amount: `${PRICES.summarize.solAmount} SOL`,
    explorerUrl: `https://explorer.solana.com/tx/${Math.random().toString(36).slice(2, 15)}?cluster=devnet`,
    isA2A: true,
    parentJobId: jobId,
    depth: 1,
  ...
  subAgentResults.push({
    ...
    cost: `${PRICES.summarize.solAmount} SOL`,
  ...
  broadcastSSE('a2a-hire', {
    hirer: 'DeepResearch Alpha',
    worker: 'SentimentAI',
    cost: PRICES.sentiment.solAmount,
  ...
  const subPayment2: PaymentLog = {
    ...
    transaction: `a2a_${Math.random().toString(16).slice(2, 14)}`,
    token: token,
    amount: `${PRICES.sentiment.solAmount} SOL`,
    explorerUrl: `https://explorer.solana.com/tx/${Math.random().toString(36).slice(2, 15)}?cluster=devnet`,
    isA2A: true,
    parentJobId: jobId,
    depth: 1,
  ...
  subAgentResults.push({
    ...
    cost: `${PRICES.sentiment.solAmount} SOL`,
  ...
  if (researchAgent) {
    ...
    researchAgent.totalEarned += PRICES.research.solAmount;
  ...
  res.json({
    ...
    recursiveDepth: 1,
    totalCostIncludingSubAgents: PRICES.research.solAmount + PRICES.summarize.solAmount + PRICES.sentiment.solAmount + (isDataQuery ? 0.02 : 0),
...
app.post('/api/agent/code', createPaidRoute(PRICES.coding), async (req: Request, res: Response) => {
  ...
  broadcastSSE('a2a-hire', {
    ...
    cost: PRICES.codeExplain.solAmount,
  ...
  const subPayment: PaymentLog = {
    ...
    transaction: `a2a_${Math.random().toString(16).slice(2, 14)}`,
    token: token,
    amount: `${PRICES.codeExplain.solAmount} SOL`,
    explorerUrl: `https://explorer.solana.com/tx/${Math.random().toString(36).slice(2, 15)}?cluster=devnet`,
    isA2A: true,
    parentJobId: jobId,
    depth: 1,
  ...
  if (codingAgent) {
    ...
    codingAgent.totalEarned += PRICES.coding.solAmount;
  ...
  res.json({
    ...
    selfReview: {
      ...
      cost: `${PRICES.codeExplain.solAmount} SOL`,
      payment: subPayment,
    },
    totalCostIncludingSubAgents: PRICES.coding.solAmount + PRICES.codeExplain.solAmount,
...
interface AgentExecutionResult {
  ...
  finalAnswer: string;
  totalCost: { SOL: number; USDC_lamports: number };
  a2aDepth: number;
  protocolTrace: Array<{
    step: string;
    httpStatus: number;
...
function autonomousHiringDecision(
  ...
  // Sort by efficiency score: (reputation / 100) * (1 / (priceSOL + 0.001)) — favors high-rep, low-cost
  ...
  const reason = `Selected ${chosen.name} (Rep: ${chosen.reputation}/100, Cost: ${chosen.priceSOL} SOL, Efficiency: ${scored[0].score.toFixed(1)}). ` +
    (alternatives.length > 0
      ? `Rejected ${alternatives[0].name} (Rep: ${alternatives[0].reputation}, Cost: ${alternatives[0].priceSOL} SOL) — lower efficiency score.`
...
async function runManagerAgent(
  ...
  const totalCost = { SOL: 0, USDC_lamports: 0 };
  let a2aDepth = 0;
  ...
  const toolsList = agentRegistry.map(agent => {
    return `- "${agent.id}": ${agent.category} Agent | Cost: ${agent.priceSOL} SOL | Reputation: ${agent.reputation}/100`;
  }).join('\n');

  const plannerPrompt = `You are the MANAGER AGENT of an autonomous AI economy on Solana blockchain.
...
Return ONLY valid JSON:
{
  ...
  for (const tc of (llmPlan.toolCalls || [])) {
    ...
    if (!price) {
      ...
      if (extAgent) {
        price = {
          solAmount: getDiscountedPrice(extAgent.priceSOL, userReputation),
          usdcLamports: extAgent.priceUSDC,
    ...
    // ── Budget Guard ──
    if (totalCost.SOL + price.solAmount > budgetLimit) {
      console.warn(`[BUDGET GUARD] Transaction blocked. Cost ${price.solAmount} exceed remaining budget of ${budgetLimit - totalCost.SOL} SOL`);
      results.push({
        tool: toolId,
        result: null,
        error: `Budget limit reached (${budgetLimit} SOL). Arbitrator Agent could be hired to request a budget increase.`
      });
      plan.push(`[GUARD] Blocked ${toolId}: Budget Limit (${budgetLimit} SOL) exceeded.`);
      continue;
    }
    ...
    if (clientId) {
      sendSSETo(clientId, 'step', {
        ...
        detail: `${price.solAmount} SOL | Rep: ${hiring.chosen?.reputation || 'N/A'}/100`,
        status: 'active',
      });
    }

    totalCost.SOL += price.solAmount;
    totalCost.USDC_lamports += price.usdcLamports;
    ...
    if (process.env.SIMULATION_MODE !== 'true') {
      try {
        ...
        const apiRes = await axios.post(`${selfUrl}${endpoint}?token=${token}`, tc.params, {
          headers: realSignature ? { 'x-solana-signature': realSignature } : {},
        });

        payment = {
          transaction: realSignature || `sim_${Math.random().toString(36).slice(2, 15)}`,
          token,
          amount: `${price.solAmount} SOL`,
          explorerUrl: getSolanaExplorerUrl(realSignature || `sim_${Math.random().toString(36).slice(2, 15)}`, NETWORK),
        };
        ...
        if (data.subAgentHires) {
          ...
          totalCost.SOL += (data.totalCostIncludingSubAgents || 0) - price.solAmount;
        }
        ...
        // ── On-chain settlement: record completed job → updates reputation ──
        if (agentKeypair && solanaConnection) {
          const anchorClient = getAnchorClient(solanaConnection, agentKeypair);
          // Ensure the worker agent exists on-chain (idempotent)
          await anchorClient.ensureAgentRegistered(
            agentName,
            endpoint,
            solToLamports(price.solAmount),
            price.category || 'general'
          ).catch(() => {});
          // Record the completed job → boosts reputation on-chain
          const onChainResult = await anchorClient.recordCompletedJob(
            agentKeypair.publicKey,
            price.category || 'general',
            solToLamports(price.solAmount)
          );
          if (onChainResult) {
            // Attach on-chain job ID to the payment for full traceability
            payment.onChainJobId = onChainResult.jobId.toString();
            payment.onChainTx = onChainResult.txSignature;
            payment.onChainExplorerUrl = getSolanaExplorerUrl(onChainResult.txSignature, NETWORK);
            // Add to protocol trace
            protocolTrace.push({
              step: `On-chain Settlement → Job #${onChainResult.jobId} (Anchor)`,
              httpStatus: 200,
              headers: {
                'x-anchor-program': '5383AVU3XCHu2L4dEVZVGtitekZEuaFBFoxgnFQJJmmB',
                'x-job-id': onChainResult.jobId.toString(),
                'x-settlement-tx': onChainResult.txSignature,
              },
              timestamp: new Date().toISOString(),
            });
            console.log(`[ANCHOR] Reputation updated on-chain for ${agentName} (job #${onChainResult.jobId})`);
          }
        }
        ...
        if (hiring.alternatives.length > 0) {
          for (let retry = 0; retry < Math.min(MAX_RETRIES, hiring.alternatives.length); retry++) {
            ...
            plan.push(`[SELF-HEAL] ${agentName} failed. Retrying with ${fallbackName} (Rep: ${fallbackAgent.reputation}, Cost: ${fallbackAgent.priceSOL} SOL)`);
            ...
            try {
              ...
              payment = {
                ...
                token: token || 'SOL',
                amount: `${fallbackAgent.priceSOL} SOL`,
                explorerUrl: `https://explorer.solana.com/tx/${Math.random().toString(36).slice(2, 15)}?cluster=devnet`,
                selfHealed: true,
              ...
              totalCost.SOL += fallbackAgent.priceSOL;
        ...
        if (!healed) {
          ...
          console.warn(`[FALLBACK] Self-healing failed for ${toolId}. Using simulation.`);
      ...
      if (toolId === 'research') {
        ...
        subHires.push({ agent: 'Summarizer Pro', task: 'Condense findings', cost: `${PRICES.summarize.solAmount} SOL`, payment: subPay });
        const subPay2 = createL2Settlement('DeepResearch Alpha', 'SentimentAI', PRICES.sentiment, token, 1);
        subHires.push({ agent: 'SentimentAI', task: 'Tone analysis', cost: `${PRICES.sentiment.solAmount} SOL`, payment: subPay2 });
        totalCost.SOL += PRICES.summarize.solAmount + PRICES.sentiment.solAmount;
        a2aDepth = Math.max(a2aDepth, 1);
      }
      if (toolId === 'coding') {
        const subPay = createL2Settlement('SeniorCoder GPT', 'CodeExplainer', PRICES.codeExplain, token, 1);
        subHires.push({ agent: 'CodeExplainer', task: 'Quality review', cost: `${PRICES.codeExplain.solAmount} SOL`, payment: subPay });
        totalCost.SOL += PRICES.codeExplain.solAmount;
        a2aDepth = Math.max(a2aDepth, 1);
      }
    ...
    if (clientId) {
      sendSSETo(clientId, 'step', {
        label: `Hiring ${agentName}`,
        detail: `Paid ${price.solAmount} SOL ✓`,
        status: 'complete',
      });
    }
    ...
    if (registryAgent) {
      ...
      registryAgent.totalEarned += price.solAmount;
    }
  }
    ...
    try {
      if (groq) {
        ...
        finalAnswer = successResults.map(r => {
          ...
          if (r.result?.summary) return `**${r.tool}**: ${r.result.summary}`;
  ...
  plan.push(`Total cost: ${totalCost.SOL.toFixed(4)} SOL`);
  ...
  return {
    ...
    finalAnswer,
    totalCost: {
      SOL: Math.round(totalCost.SOL * 10000) / 10000,
      USDC_lamports: totalCost.USDC_lamports,
    },
    a2aDepth,
    protocolTrace,
  };
...
function formatResearchForAnswer(data: { summary: string; sources: Array<{ title: string; url: string }>; key_findings: string[] }): string {
  const summary = data.summary.length > 500 ? data.summary.slice(0, 500) + '...' : data.summary;
...
async function simulateToolResult(toolId: string, params: any, query: string): Promise<any> {
  switch (toolId) {
    ...
    case 'research': {
      const wiki = await fetchWikipediaResearch(params.query || query);
      return formatResearchForAnswer(wiki);
    }
    case 'coding':
...
app.post('/api/agent/query', createPaidRoute(PRICES.manager), async (req: Request, res: Response) => {
  try {
    ...
    const result = await runManagerAgent(query, token || 'SOL', clientId, options);
    res.json(result);
  } catch (err) {
...
app.post('/api/agent/arbitrate', createPaidRoute(PRICES.arbitrator), async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({
      ...
      network: NETWORK,
      explorerUrl: getSolanaExplorerUrl('', NETWORK),
    });
  } catch (err: any) {
...
app.post('/api/kaggleingest', createPaidRoute(PRICES.kaggleingest), async (req: Request, res: Response) => {
  ...
  (async () => {
    ...
    for (let i = 0; i < activeAgents.length; i++) {
      ...
      const p: PaymentLog = {
        ...
        token: swapNeeded ? 'USDC' : 'SOL',
        amount: swapNeeded ? '0.005 USDC' : `${agent.priceSOL} SOL`,
    ...
    sendSSETo(clientId, 'thought', {
      content: "[STRESS TEST COMPLETE] Hive-mind synchronized. Symbisol-Engine stable at peak load."
    });
  })();
});
...
app.listen(PORT, HOST, () => {
  console.log('');
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║  SYMBISOL — x402 Autonomous Agent Economy                   ║');
  console.log('║  Agent-to-Agent Micropayment Marketplace on Solana         ║');
  console.log('╠══════════════════════════════════════════════════════════════╣');
  console.log(`║  Server      : http://${HOST}:${PORT}`);
  console.log(`║  Network     : ${NETWORK}`);
  console.log(`║  RPC         : ${SOLANA_RPC_URL}`);
  console.log(`║  Agents      : ${agentRegistry.length} registered`);
  console.log(`║  Wallet      : ${SERVER_ADDRESS}`);
  console.log('╠══════════════════════════════════════════════════════════════╣');
  console.log('║  Paid Endpoints (Worker Agents):');
  Object.entries(PRICES).forEach(([id, p]) => {
    console.log(`║    ${id.padEnd(12)} ${p.solAmount.toString().padEnd(6)} SOL | ${p.usdcLamports.toString().padEnd(5)} microUSDC`);
  });
  console.log('╠══════════════════════════════════════════════════════════════╣');
