/**
 * ═══════════════════════════════════════════════════════════════════════════
 * SYMBISOL — Autonomous x402 Agent (CLI + Programmatic)
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * An AI agent that:
 ...
 *   5. Pays each Worker Agent via x402 (SOL/USDC) on Solana
...
async function x402Post(endpoint: string, data: any, lamports: number, recipient?: string) {
  const firstRes = await axios.post(`${SERVER_URL}${endpoint}`, data, {
  ...
  const tx = new Transaction().add(
    SystemProgram.transfer({
      ...
      lamports: lamports || payment.amount,
    })
  );
  ...
  const retryRes = await axios.post(`${SERVER_URL}${endpoint}`, data, {
...
async function discoverTools(): Promise<Tool[]> {
  ...
  try {
    ...
    availableTools.forEach(t => {
      ...
      console.log(
        `  ├─ ${t.name.padEnd(22)} ${t.price.SOL.toString().padEnd(6)} SOL | Rep: ${t.reputation}/100 | Jobs: ${t.jobsCompleted}${subLabel}`
      );
    });
...
function evaluateWorkers(tools: Tool[]): Tool[] {
  return tools.sort((a, b) => {
    const scoreA = (a.reputation * a.reputation) / (a.price.SOL * 10000 || 1);
    const scoreB = (b.reputation * b.reputation) / (b.price.SOL * 10000 || 1);
...
function makeHiringDecision(toolId: string, tools: Tool[]): HiringDecision | null {
  ...
  const costEfficiency = tool.price.SOL > 0
    ? Math.round((tool.reputation * tool.reputation) / (tool.price.SOL * 10000))
    : 0;

  let reason: string;
  if (alternatives.length > 0) {
    const alt = alternatives[0];
    const altEfficiency = alt.price.SOL > 0
      ? Math.round((alt.reputation * alt.reputation) / (alt.price.SOL * 10000))
      : 0;
    ...
    if (costEfficiency >= altEfficiency) {
      reason = `Selected ${tool.name} (Efficiency: ${costEfficiency}) over ${alt.name} (Efficiency: ${altEfficiency}). ` +
        `Reason: Better cost-reputation ratio at ${tool.price.SOL} SOL with ${tool.reputation}/100 reputation.`;
    } else {
      reason = `Selected ${tool.name} (Cost: ${tool.price.SOL} SOL, Rep: ${tool.reputation}/100) — ` +
    ...
    reason = `Hiring ${tool.name}: Only available specialist in "${tool.category}" category. ` +
      `Cost: ${tool.price.SOL} SOL, Rep: ${tool.reputation}/100.`;
  }
...
async function planToolCalls(query: string, tools: Tool[]): Promise<AgentPlan> {
  const toolsDescription = tools.map(t =>
    `- ID: "${t.id}" | Name: "${t.name}" | Cost: ${t.price.SOL} SOL | Rep: ${t.reputation}/100 | Cat: ${t.category} | ${t.canHireSubAgents ? 'CAN HIRE SUB-AGENTS' : 'Worker'}\n  Description: ${t.description}\n  Params: ${JSON.stringify(t.params)}`
  ).join('\n\n');

  const systemPrompt = `You are the MANAGER AGENT of SYMBISOL — an autonomous AI economy on Solana blockchain.

You have a BUDGET and must hire Worker Agents via x402 micropayments.
Each hire costs real SOL tokens on the Solana blockchain.
...
async function executeTool(
  ...
  console.log(`[AGENT] [PAY] Hiring ${tool.name} (${tool.price.SOL} SOL, Rep: ${tool.reputation}/100)`);
  console.log(`[AGENT] [NOTE] Reason: ${reason}`);

  try {
    const amountLamports = Math.round(tool.price.SOL * LAMPORTS_PER_SOL);
    const res = await x402Post(tool.endpoint, params, amountLamports, process.env.SERVER_ADDRESS);
    ...
    const sig = res.headers?.['x-solana-signature'] || '';
    if (sig) {
      result.payment = {
        transaction: sig,
        token,
        amount: `${tool.price.SOL} SOL`,
        explorerUrl: `https://explorer.solana.com/tx/${sig}?cluster=${NETWORK}`,
      };
      console.log(`[AGENT] [OK] Paid ${tool.price.SOL} SOL | tx: ${sig}`);
    }
    ...
    for (let retry = 0; retry < Math.min(MAX_RETRIES, alternatives.length); retry++) {
      ...
      console.log(`[AGENT] [SELF-HEAL] Fallback: ${fallback.name} (Rep: ${fallback.reputation}, Cost: ${fallback.price.SOL} SOL)`);

      try {
        const fallbackAmount = Math.round(fallback.price.SOL * LAMPORTS_PER_SOL);
        const fallbackRes = await x402Post(fallback.endpoint, params, fallbackAmount, process.env.SERVER_ADDRESS);
        ...
        const fallbackSig = fallbackRes.headers?.['x-solana-signature'] || '';
        if (fallbackSig) {
          healedResult.payment = {
            transaction: fallbackSig,
            token,
            amount: `${fallback.price.SOL} SOL`,
            explorerUrl: `https://explorer.solana.com/tx/${fallbackSig}?cluster=${NETWORK}`,
          };
          console.log(`[AGENT] [SELF-HEAL] Recovered via ${fallback.name} | Paid ${fallback.price.SOL} SOL`);
        }
...
async function processQuery(
  ...
  for (const call of plan.toolCalls) {
    ...
    if (result.success && tool) {
      ...
      if (result.data?.totalCostIncludingSubAgents) {
        const subCost = result.data.totalCostIncludingSubAgents - tool.price.SOL;
        a2aCost += subCost;
        totalCost += subCost;
  ...
  console.log('');
  console.log(`[AGENT] [PAY] Total cost: ${totalCost.toFixed(4)} SOL (incl. ${a2aCost.toFixed(4)} SOL A2A)`);
...
async function startRepl() {
  console.log('');
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║              SYMBISOL — x402 AUTONOMOUS AGENT                ║');
  console.log('║           Agent-to-Agent Economy on Solana                  ║');
  console.log('╠══════════════════════════════════════════════════════════════╣');
  console.log(`║  Server  : ${SERVER_URL.padEnd(49)}║`);
  console.log(`║  Wallet  : ${keypair.publicKey.toBase58().padEnd(49)}║`);
    ...
    rl.question('\n[SYMBISOL] > ', async (input) => {
      ...
      if (trimmed === 'tools') {
        ...
        for (const t of availableTools) {
          ...
          console.log(`│ ${t.name.padEnd(22)} ${t.price.SOL.toString().padEnd(7)} SOL | Rep: ${t.reputation.toString().padEnd(3)}/100 | ${t.category}${sub} │`);
        }
        console.log('└───────────────────────────────────────────────────────────┘');
        prompt();
        return;
      ...
      if (trimmed === 'registry') {
        try {
          ...
          for (const a of res.data.agents) {
            console.log(`│ ${a.name.padEnd(22)} Rep: ${a.reputation.toString().padEnd(3)}/100 | Jobs: ${a.jobsCompleted.toString().padEnd(5)} | Earned: ${a.totalEarned.toFixed(1)} SOL │`);
          }
      ...
      if (trimmed === 'demo') {
        ...
        await processQuery('Research the x402 protocol on Solana, summarize the findings, and check the weather in Tokyo');
        prompt();
        return;
      }
