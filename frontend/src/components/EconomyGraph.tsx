...
const NODE_COLORS: Record<string, { bg: string; border: string; glow: string }> = {
  user:    { bg: '#ffffff', border: '#16a34a', glow: 'rgba(22,163,74,0.15)' },
  manager: { bg: '#ffffff', border: '#15803d', glow: 'rgba(21,128,61,0.15)' },
...
export default function EconomyGraph({ refreshTrigger = 0 }: { refreshTrigger?: number }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // User → Manager line
      drawEdge(ctx, userNode, managerNode, false, tick, '#16a34a');
      ...
      workerNodes.forEach(wn => {
        ...
        const color = isA2A ? '#059669' : '#16a34a'; // Teal for A2A, Green for Standard
      ...
      a2aEdges.forEach(e => {
        ...
        if (fromNode && toNode) {
          drawCurvedEdge(ctx, fromNode, toNode, tick, '#059669');
        }
      });
      ...
      if (edges.length > 0) {
        ...
        if (fromNode && toNode) {
          ...
          ctx.fillStyle = activeEdge.isA2A ? '#059669' : '#16a34a';
          ctx.fill();
        }
      }
    ...
    <div className="glass-panel" style={{ padding: 16 }}>
      ...
      <div style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '1px solid #e2e8f0', background: '#ffffff' }}>
        ...
        <div style={{
          ...
          {[
            { color: '#16a34a', label: 'User → Manager' },
            { color: '#16a34a', label: 'Manager → Worker' },
            { color: '#059669', label: 'A2A Recursive' },
          ].map(l => (
