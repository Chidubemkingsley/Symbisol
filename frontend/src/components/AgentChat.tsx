...
import { Send, Terminal, Loader2, Shield, Zap, DollarSign, Activity, Share2, Wallet } from 'lucide-react';
        ...
        return (
          <div key={idx} style={{ marginBottom: 8 }}>
            <div style={{
              ...
              <span style={{ color: 'var(--text-muted)', fontSize: '0.7rem' }}>— {hire.cost} {hire.currency || 'SOL'}</span>
            </div>
...
export default function AgentChat({ onNewPayments, onProtocolTrace }: Params) {
  ...
  useEffect(() => {
    let id = localStorage.getItem('symbisol_client_id');
    if (!id) {
      id = `client_${Math.random().toString(36).substring(2, 11)}`;
      localStorage.setItem('symbisol_client_id', id);
    }
    clientId.current = id;
  }, []);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      const sseUrl = `${(process.env.NEXT_PUBLIC_API_URL || 'https://symbisol.onrender.com').replace(/\/$/, '')}/api/agent/events?clientId=${clientId.current}`;
      ...
      sse.addEventListener('a2a-hire', (event) => {
        try {
          ...
          setMessages(prev => [...prev, {
            role: 'system',
            content: `🔄 **Recursive Hire:** ${data.hirer} hired **${data.worker}** for ${data.cost} SOL.`,
            depth: data.depth || 1
          }]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Step 1: Send query — expect 402 Payment Required
      const firstRes = await fetch(`${API}/api/agent/query`, {
        method: 'POST',
      ...
      if (firstRes.status === 402) {
        ...
        if (!publicKey) {
          setMessages(prev => [...prev, {
            role: 'system',
            content: '**Wallet required.** Connect a Phantom or Solflare wallet to pay for this query ({payment.amount / LAMPORTS_PER_SOL} {payment.token}).'
          }]);
          setIsProcessing(false);
          setAgentStatus('idle');
          return;
        ...
        try {
          ...
          setMessages(prev => [...prev, {
            role: 'system',
            content: `**Payment cancelled.** ${walletErr.message || 'User rejected the transaction.'}`
          }]);
          setIsProcessing(false);
          setAgentStatus('idle');
          return;
        ...
        setMessages(prev => [...prev, {
          role: 'system',
          content: `\`x402:\` Paid ✓ [\`${signature.slice(0, 12)}...\`](https://explorer.solana.com/tx/${signature}?cluster=devnet)`
        }]);

        // Step 3: Retry with payment signature
        setAgentStatus('planning');
        const retryRes = await fetch(`${API}/api/agent/query`, {
          method: 'POST',
          headers: {
      ...
      // Step 4: Handle direct 200 response (no x402, or simulation mode)
      if (!firstRes.ok) {
