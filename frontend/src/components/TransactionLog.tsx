...
interface Payment {
  ...
  // On-chain Anchor settlement fields
  onChainJobId?: string;
  onChainTx?: string;
...
export default function TransactionLog({ refreshTrigger }: Props) {
  ...
  useEffect(() => {
    const fetchPayments = async () => {
      try {
        const res = await fetch(`${API}/api/payments`, { signal: AbortSignal.timeout(5000) });
        if (!res.ok) return;
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.6rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
        ...
        <span style={{ color: payment.isA2A ? '#059669' : 'var(--accent-500)' }}>→</span>
      ...
      {/* On-chain Anchor settlement badge */}
      {payment.onChainJobId && (
        <div style={{
          marginTop: 6,
          padding: '4px 8px',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <span style={{ fontSize: '0.5rem', color: '#16a34a', fontFamily: 'var(--font-mono)', fontWeight: 700 }}>
              ANCHOR · Job #{payment.onChainJobId}
            </span>
          </div>
          {payment.onChainExplorerUrl && (
            <a
              href={payment.onChainExplorerUrl}
              target="_blank"
              rel="noreferrer"
              style={{ fontSize: '0.5rem', color: '#16a34a', textDecoration: 'none', fontFamily: 'var(--font-mono)' }}
            >
              on-chain ↗
            </a>
          )}
